...
#use with file "data/super-metroid/top-6.tsv" 
...
graph <- ggplot(data = ftable) +
	 ...
	 geom_point(data=zoast, mapping = aes(x = fdate, y = format_time,shape=player),size=3) +
	 #kottpower
   	 geom_line(data=kottpower, mapping = aes(x = fdate, y = format_time),linetype=2) +
	 geom_point(data=kottpower, mapping = aes(x = fdate, y = format_time,shape=player),size=3) +
	 #oatsngoats
     geom_line(data=oatsngoats, mapping = aes(x = fdate, y = format_time),linetype=3) +
	 geom_point(data=oatsngoats, mapping = aes(x = fdate, y = format_time,shape=player),size=3) +
	 #behemoth
     geom_line(data=behemoth, mapping = aes(x = fdate, y = format_time),linetype=4) +
	 geom_point(data=behemoth, mapping = aes(x = fdate, y = format_time,shape=player),size=3) +
	 #jack879
     geom_line(data=jack879, mapping = aes(x = fdate, y = format_time),linetype=1) +
	 geom_point(data=jack879, mapping = aes(x = fdate, y = format_time,shape=player),size=3) +
	 #wildanaconda69
     geom_line(data=wildanaconda69, mapping = aes(x = fdate, y = format_time),linetype=6) +
	 geom_point(data=wildanaconda69, mapping = aes(x = fdate, y = format_time,shape=player),size=3) +
