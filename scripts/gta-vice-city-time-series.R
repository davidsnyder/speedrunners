...
graph <- ggplot() +
      	 geom_line(data=subset(ftable,category=="ACE"),mapping = aes(x = fdate, y = format_time),color="blue") +
	 geom_point(data=subset(ftable,category=="ACE"),mapping = aes(x = fdate, y = format_time),color="blue") +
	 geom_line(data=subset(ftable,category=="No ACE"),mapping = aes(x = fdate, y = format_time),color="red") +
	 geom_point(data=subset(ftable,category=="No ACE"),mapping = aes(x = fdate, y = format_time),color="red") +
	 ggtitle(graph_title) +
...
#	 geom_text_repel(data=subset(ftable, player=="Zoast"), aes(fdate,format_time,label=player), size=3) + #label all points matching player
	 geom_text_repel(data=subset(ftable, date=="8/9/2014"), aes(fdate,format_time,label="replays discovery"),size=4,nudge_y=125,point.padding=unit(0.25,'lines'),box.padding=unit(2,'lines')) + #label all points matching date
	 geom_text_repel(data=subset(ftable, date=="8/1/2016"), aes(fdate,format_time,label="ACE discovery"),size=4,point.padding=unit(0.25,'lines'),box.padding=unit(2,'lines')) + #label all points matching date
